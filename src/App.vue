...
const startNewGame = async (skipConfirm = false) => {
  if (isGameFinished.value && !isGameSaved.value) {
    if (!await showConfirm("대국 결과가 아직 기록되지 않았습니다. 정말 새 게임을 시작하시겠습니까?")) {
      return;
    }
    skipConfirm = true;
  }

  if (skipConfirm || await showConfirm("현재 진행 중인 게임이 초기화됩니다. 새 게임을 시작하시겠습니까?")) {
    resetAll();
    showModal('choose_seat');
  }
};
...
const invalidateGame = async (index: number) => {
  const confirmInvalidate = await showConfirm(`${index + 1}회전 대국 기록을 무효(삭제) 처리하시겠습니까?`);
...
<div class="background" @dblclick.self="toggleFullScreen()" @click="onBackgroundClick">
  ...
  <!-- 커스텀 컨펌 다이얼로그 (주소창 방지용) -->
  <Transition name="modal-fade">
    <div v-if="confirmState.isOpen" class="custom-confirm-overlay" @click="handleConfirm(false)">
      <div class="custom-confirm-card" @click.stop>
        <div class="custom-confirm-message" v-html="confirmState.message.replace(/\n/g, '<br>')"></div>
